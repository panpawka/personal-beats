...
A newsroom-of-one is a workflow that only makes sense now that the tools have changed. It needs long-running reasoning, real code execution against a live web, and persistent memory across runs — all three are exactly what Claude Managed Agents now provides as a primitive. Two years ago you could fake the demo. You could not ship the product. The interface — *one sentence in, a paper out, on your schedule* — does not have a name yet, because the substrate that lets you build it is two months old.

Kasper follows geopolitical situation daily. Same orchestration, different brief - both verified, both shipped before breakfast
...
**Surprises.** The clarification gate was the moment that taught us most. An earlier prompt told the Designer to "ask if anything is unclear"; the model asked too much, and the product felt like an interview. Flipping the prompt to *lean toward proceeding, only ask if two reasonable answers would produce substantially different newsletters* — and showing concrete *DO/DO-NOT-ASK* examples — produced markedly better behaviour on Opus 4.7 than on earlier Opuses I'd worked with. The model takes calibration instructions and applies them with taste.
...
This split — Wasp owns product data, Managed Agents owns the agent brain — is what let us ship something more ambitious than a chat wrapper inside a hackathon week.
